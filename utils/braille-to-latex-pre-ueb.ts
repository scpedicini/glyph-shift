...
const convertBraille2 = (line: string): string => {
    let result = '';
    ...
    result = result.replaceAll(/\bo'clock\b/g, "{o'clock}");
    ...
    result = result.replaceAll(/{(by|into|to)}\s+/g, '{$1}');

    return convertBraille1(result);
};

export { convertBraille2, convertBraille1 };
